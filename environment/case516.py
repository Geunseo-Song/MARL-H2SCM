...
class HydroRefuelSys(MultiAgentEnv):
    ...
    def __init__(self, config=None):
        ...
        super().__init__()  # class 'HydroRefuelSys' inherits the properties of class 'MultiAgentEnv'

        # data import
        self.hload, self.PVrate, self.MGpie = [], [], []
        for i in range(self.num_HRS):
            a = '/home/sgs650/MARLlib/CaseStudy/data{}.csv'.format(i)
            if os.path.isfile(a):  # Check if the file exists.(In case the file exists.)
                b = pd.read_csv(a)
                self.hload.append([b["hload"]])
                self.PVrate.append([b["r"]])
                self.MGpie.append([b["Pie"]])
            else:  # In case the file does not exist.
                # print(f"There is no data for HRS{i}. Use HRS0 data." )
                a = '/home/sgs650/MARLlib/CaseStudy/data0.csv'
                b = pd.read_csv(a)
                self.hload.append([b["hload"]])
                self.PVrate.append([b["r"]])
                self.MGpie.append([b["Pie"]])
        ...
        self.B_CO2_total = 0
        self.X_CO2_total = 0
        # self.H_pie = self.G_pie * self.hydro_G + self.B_pie * self.hydro_B + self.X_pie * self.hydro_X
        ...
        self.diff_SOH = [0 for i in range(self.num_HRS)]  # SOHmax - SOH
        ...
        # HDS parameter (Set up to determine hydroM pricing.)
    ...
    def step(self, action_dict):
        ...
        if "HDS" in action_dict:
            # print(action_dict["HDS"])
            return self._HDS_step(action_dict["HDS"])
        else:
            # print("step for HRS starts.............................................")
            # print(list(action_dict.values()))
            # print(list(action_dict.values())[0])
    ...
    def _HDS_step(self, action):
        ...
        # print(self.H_pie[0])
        # print(self.SOH[0])
        # print(self.hload[0][0][0])
        # print(self.MGpie[0][0][0])
        # print(self.powerPV[0][0][0])
        for ag in range(self.num_HRS):
    ...
    def _HRS_step(self, action):
        ...
        # print("timestep", self.steps_remaining_at_level)
        rew, obs, done = {}, {}, {}
        ...
        # print(hydro_G, hydro_B, hydro_X, powerMG, powerDG)
        ...
        # print("hydroGAccum", self.hydroGAccum)
        self.hydroGAvail = self.hydroG - sum(self.hydroGAccum)
        # print("hydroGAvail", self.hydroGAvail)
        self.hydroBAccum[0] += hydro_B
        # print("hydroBAccum", self.hydroBAccum)
        self.hydroBAvail = self.hydroB - sum(self.hydroBAccum)
        # print("hydroBAvail", self.hydroBAvail)
        self.hydroXAccum[0] += hydro_X
        # print("hydroXAccum", self.hydroXAccum)
        self.hydroXAvail = self.hydroX - sum(self.hydroXAccum)
        # print("hydroXAvail", self.hydroXAvail)
        ...
        # print("Pcom:", Pcom, "Pele", Pele)
        # print("powerPV:", self.powerPV[ag][0][23 - self.steps_remaining_at_level])
        ...
        # print("hydroRemain", hydroRemain)
        # print("SOH", self.SOH[ag])
        if hydroRemain > self.SOHmax[0] - self.SOH[0] and hydroRemain >= 0:
            if hydroRemain - (self.SOHmax[0] - self.SOH[0]) < 1:
                ...
                # print("case1-1", self.SOH, self.penalty_HRS)
            else:
                self.penalty_HRS = (hydroRemain - (self.SOHmax[0] - self.SOH[0])) * 10 * (
                            1 + (self.G_pie[0] + self.B_pie[0] + self.X_pie[0]) / 3)
                # print("case1-2", self.SOH, self.penalty_HRS)
            ...
            if -(self.SOH[0] + hydroRemain) < 1:
                self.penalty_HRS = -(self.SOH[0] + hydroRemain) * (
                            1 + (self.G_pie[0] + self.B_pie[0] + self.X_pie[0]) / 3)
                # print("case4-1", self.SOH, self.penalty_HRS)
            else:
                self.penalty_HRS = -(self.SOH[0] + hydroRemain) * 10 * (
                            1 + (self.G_pie[0] + self.B_pie[0] + self.X_pie[0]) / 3)
                # print("case4-2", self.SOH, self.penalty_HRS)
            self.SOH[0] = 0
            # print("SOH", self.SOH)
        else:
            # print(hydroRemain, self.SOH[0])
            raise ValueError
        ...
        # print(self.H_pie[ag])
        # print(self.SOH[ag])
        # print(self.hload[ag][0][24 - self.steps_remaining_at_level])
        # print(self.MGpie[ag][0][24 - self.steps_remaining_at_level])
        # print(self.powerPV[ag][0][24 - self.steps_remaining_at_level])
        ...
        # print("Pcom:", Pcom, "Pele", Pele)
        # print("powerPV:", self.powerPV[1][0][23 - self.steps_remaining_at_level])
